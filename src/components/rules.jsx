import React from "react";

export const Rules = () => {
  return (
    <>
      <div className="text-2xl" id="rules">
        <h1 className="text-4xl font-bold py-4">Normas</h1>
        <p>
          Antes de iniciar con las normas es importante dejar claro, esta es una
          natillera exclusivamente familiar y/o amigos de confianza, no tiene
          fines lucrativos para ninguna parte, y fue desarrollada solo con el
          fin de integrar de alguna manera a la familia o persona independiente
          en un ahorro navideño. Es administrada por personas responsables.
        </p>
        <p className="mt-5">
          si desea hacer parte de este mundo de ahorro familiar bienvenido a
          natilleando.
        </p>
      </div>
    </>
  );
};
